...
if __name__ == '__main__':
    try:
        ...
        if sys.argv[1] in ('help', '-?'):
            ...
            createFile(sys.argv[2], sys.argv[3])   # Create file
        elif sys.argv[1] in ('project', '-P'):
            createProject(sys.argv[2], sys.argv[3])   # Crate project
        elif sys.argv[1] in ('append', '-a'):
            appendTemplate(sys.argv[2])   # Append a template
        elif sys.argv[1] in ('remove', '-R'):
            removeTemplate(sys.argv[2])   # Remove a template
        elif sys.argv[1] in ('list', '-l'):
            showList()   # Show list of templates

        # Show version info
        elif sys.argv[1] in ('version', '-v'):
            print(
                f'Version: \033[1;32m{__version__}\033[m\n'
        ...
        # In case of user digit someting wrong
            ...
            print(
                "Sorry, this command doesn't exist. digit \033[1;42mefc -?\033[m for commands"
            )

    # In case of dont have enough arguments
    except IndexError:
        print(
            '\033[1mMissing arguments...\033[m\n'
    ...
    # In case of a unknown error happens
        ...
        print(
            f'\033[1;31mAn error has accured, the name of this error is {Except}\033[m\n'
